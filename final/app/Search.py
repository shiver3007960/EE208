...
class Query:
    ...
    @staticmethod
    def presentQuery(URL = None, storeDir = "URLdir", field = None, contentsGeneral = '', highlighter = None, analyzer = WhitespaceAnalyzer(), doc = None):
    ...
    def run(indexDir, query, field):
        ...
        #分词器
        # Must using standard Analyzer because the content retrieved is raw rather than Processed
    ...
    @staticmethod
    def runForWeb(indexDir, query, field, mainContent, VM):
        ...
        if isinstance(query, str) and len(query) > 0:
            ...
            print(res)
            Query.formatForHTML(indexDir, res, field, mainContent, VM)
            

    @staticmethod
    def formatForHTML(indexDir, query, field, mainContent, VM):
        if query == '':
            return
        # 索引目录
        VM.attachCurrentThread()
        ...
        #高亮显示
        htmlFormatter = SimpleHTMLFormatter("<strong>", "</strong>")
        queryScorer = QueryScorer(query)
        highlighter = Highlighter(htmlFormatter, queryScorer)
        ...
        for scoreDoc in scoreDocs:
            ...
            res = Query.presentQuery(URL, field=field, contentsGeneral = contentsGeneral, highlighter = highlighter, analyzer=analyzer, doc = doc)
            if res is not None:
                mainContent['the full URL'].append(URL)
                mainContent['scheme+netloc'].append(parse.urljoin(parse.urlparse(URL).scheme, parse.urlparse(URL).netloc))
                mainContent['path'].append(' > '.join(parse.urlparse(URL).path.split('/')))
                
                title = doc.get("htmlTitle")
                title = re.sub(formatter, '', title)
                content, filepath, contentsGeneral = res[0], res[1], res[2]
                mainContent['theMainDescription'].append(content)
                mainContent['URLTitle'].append(title)
                mainContent['imgsrc'].append('')
                mainContent['Additional information'].append('')
